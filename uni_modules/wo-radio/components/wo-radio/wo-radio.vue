<template>
	<view :style="{'--radio-primary': props.styleObj.primary, '--radio-selected-bg': props.styleObj.selectedBg, '--radio-unselected-color': props.styleObj.unselectedRadioBg, '--radio-height': `${props.styleObj.height}px`}">
		<view style="position: relative;">
			<view id='radios' v-for="(item, index) in state.items" :key="index" @click="onChange(item, index)" class="item-layout" :class="{ itemActive: state.current === index}">
				<view class="circle" :class="{ circleActive: state.current === index }"></view>
				<view>
					<slot :data="item">{{ item.name }}</slot>
				</view>
			</view>
			<view class="layer" :style="{transform: `translateY(${state.moveTo}px)`, height: `${30 + props.styleObj.height}px`}"></view>
		</view>
	</view>
</template>
