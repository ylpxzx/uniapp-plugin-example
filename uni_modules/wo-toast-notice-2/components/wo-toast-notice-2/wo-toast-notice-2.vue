...
export default {
  ...
  methods: {
    showToast(data) {
      const info = this.toastTypes[data && data.type];
      if (!info) return;
      ...
      setTimeout(() => {
        this.$set(this.toasts[index], "visible", true);
      }, 10);
      if (this.timeLength === 0) {
        return;
      }
      const delay = this.timeLength + index * 800;
      setTimeout(() => this.closeToast(id), delay);
    },
    closeToast(id) {
      const idx = this.toasts.findIndex((t) => t.id === id);
      if (idx !== -1) {
        this.$set(this.toasts[idx], "visible", false);
        setTimeout(() => {
          const removeIdx = this.toasts.findIndex((t) => t.id === id);
          if (removeIdx !== -1) this.toasts.splice(removeIdx, 1);
        }, 500);
      }
    },
  },
